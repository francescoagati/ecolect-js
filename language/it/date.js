...
module.exports = function(language) {
	...
	return new Parser(language)
		...
		.add([ 'questo', dayOfWeek ], nextDayOfWeek)
		.add([ 'questa', dayOfWeek ], nextDayOfWeek)		
		.add([ 'prossimo', dayOfWeek ], nextDayOfWeek)
		.add([ 'prossima', dayOfWeek ], nextDayOfWeek)		
		.add([ 'on', dayOfWeek ], nextDayOfWeek)

		// Expressions for describing the day, such as today and tomorrow
		.add('oggi', (v, e) => adjustedDays(currentTime(e), 0))
		.add('domani', (v, e) => adjustedDays(currentTime(e), 1))
		.add('dopodomani', (v, e) => adjustedDays(currentTime(e), 2))
		.add('ieri', (v, e) => adjustedDays(currentTime(e), -1))
		...
		.add([ month, 'nel', /^[0-9]{1,2}$/ ], v => withYear(v[0], v[1]))
		.add([ month, 'del', /^[0-9]{1,2}$/ ], v => withYear(v[0], v[1]))
		...
		// Relative dates
		.add([ integer, 'giorni' ], v => { return { relativeDays: v[0].value }})
		.add([ integer, 'mesi' ], v => { return { relativeMonths: v[0].value }})
		.add([ integer, 'settimane' ], v => { return { relativeDays: v[0].value * 7 }})
